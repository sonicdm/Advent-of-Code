...
def main():
    ...
    print("Calculating fuel usage for data")
    index, fuel = align_coords(example_data, increment=0)
    print(f'Calculated fuel usage for data: {fuel} at index: {index}')
    
    print("Calculating fuel usage for data with increment")
    index, fuel = align_coords(example_data, increment=1)
    print(f'Calculated fuel usage for data: {fuel} at index: {index}')
        
def align_coords(data: list, increment=0) -> tuple[int,int]:
    """
    ...
    with tqdm(total=len(data), leave=False, desc=f"Destination: {destination} Fuel: {sum(fuel_by_origin.values())}") as pbar:
        if False:
            ...
            for origin in data:
                ...
                origin_cost = cost(origin,destination)
                fuel_by_origin[origin] = origin_fuel
                pbar.update()
